...
export function ArtisanSignup() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/applications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.name, phone: form.phone, trade: form.trade, area: form.area, years_experience: Number(form.years), documents: Object.keys(documents) }) });
  ...
  return <main className="signup-shell">
    ...
    <form className="signup-form" onSubmit={(event) => { event.preventDefault(); if (step < 1) setStep(1); else void submit(); }}>
      ...
      {step === 0 ? <div className="form-panel"><h2>Create your professional profile</h2>
        ...
        {evidence.map((item) => <label className="document-upload" key={item}><FileUp /><span><strong>{item}</strong><small>{documents[item] || "PDF, JPG or PNG · up to 10 MB"}</small></span><em>{documents[item] ? "Added" : "Choose file"}</em><input type="file" required accept=".pdf,image/*" onChange={(event) => setDocuments((current) => ({ ...current, [item]: event.target.files?.[0]?.name || "" }))} /></label>)}
      </div>}
