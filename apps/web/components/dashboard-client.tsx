...
type User = { name:string; role:string;email?:string;phone?:string } | null;
...
export function DashboardClient({initialSection="overview"}:{initialSection?:string}) {
  ...
  return <main className="dashboard-shell">
    ...
    <section className="dashboard-main"><header><div><span>Private {artisanMode ? "artisan" : "client"} workspace</span><h1>{section === "overview" ? `Welcome${user?.name ? `, ${user.name}` : ""}.` : nav.find(([id]) => id === section)?.[1] || "Account profile"}</h1></div><div className="dash-head-actions"><button aria-label="Notifications" onClick={() => notify(jobs.length ? `${jobs.length} authorized jobs loaded.` : "No new job alerts.")}><Bell size={19}/>{jobs.length > 0 && <i/>}</button><span className="identity-badge">{user?.name?.split(" ").map((part) => part[0]).join("").slice(0,2) || "MM"}</span></div></header>{notice && <div className="action-toast">{notice}</div>}
      ...
      {section === "profile" && <ProfileEditor artisanMode={artisanMode} user={user} notify={notify}/>}
    </section>
  </main>;
}
...
function ProfileEditor({artisanMode,user,notify}:{artisanMode:boolean;user:User;notify:(message:string)=>void}){
  const[profile,setProfile]=useState({bio:"",skills:"",area:"Kilimani",available:false});const[loading,setLoading]=useState(artisanMode);
  useEffect(()=>{if(!artisanMode)return;void fetch("/api/marketplace/artisans/me").then(async response=>{if(response.ok){const data=await response.json();setProfile({bio:data.bio||"",skills:(data.skills||[]).join(", "),area:data.area,available:data.available})}}).finally(()=>setLoading(false))},[artisanMode]);
  const save=async()=>{if(!artisanMode){notify("Client account details are managed securely through support.");return}const response=await fetch("/api/marketplace/artisans/me",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({bio:profile.bio,skills:profile.skills.split(",").map(value=>value.trim()).filter(Boolean),area:profile.area,available:profile.available})});notify(response.ok?"Professional profile saved.":"Profile could not be saved.")};
  return <section className="dash-panel-page profile-editor"><span className="kicker">{artisanMode?"Trust passport":"Private account"}</span><h2>{artisanMode?"Professional profile":"Account profile"}</h2>{artisanMode?<>{loading&&<p>Loading profile…</p>}<label>About your work<textarea rows={5} value={profile.bio} onChange={event=>setProfile({...profile,bio:event.target.value})} placeholder="Tell clients how you work."/></label><label>Skills, separated by commas<input value={profile.skills} onChange={event=>setProfile({...profile,skills:event.target.value})} placeholder="Pipe repair, installations"/></label><label>Primary service area<input value={profile.area} onChange={event=>setProfile({...profile,area:event.target.value})}/></label><label className="availability-toggle"><input type="checkbox" checked={profile.available} onChange={event=>setProfile({...profile,available:event.target.checked})}/>Accepting new jobs</label></>:<><label>Name<input value={user?.name||""} readOnly/></label><label>Email<input value={user?.email||""} readOnly/></label><label>Phone<input value={user?.phone||""} readOnly/></label></>}<button onClick={()=>void save()} className="button button-dark">Save profile</button></section>
}

function PaymentMethods(){
  const[methods,setMethods]=useState<{id:string;method_type:string;label:string;last_four:string;is_default:boolean}[]>([]);const[phone,setPhone]=useState("");const[notice,setNotice]=useState("");
  const load=()=>void fetch("/api/marketplace/payment-methods").then(async response=>{if(response.ok)setMethods(await response.json())});useEffect(load,[]);
  const add=async()=>{const digits=phone.replace(/\D/g,"");const response=await fetch("/api/marketplace/payment-methods",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({method_type:"mpesa",provider:"safaricom",label:`M-Pesa · ${digits.slice(-4)}`,last_four:digits.slice(-4),is_default:!methods.length})});setNotice(response.ok?"M-Pesa method saved.":"Payment method could not be saved.");if(response.ok){setPhone("");load()}};
  return <div className="payment-methods"><div className="card-heading"><div><span className="kicker">Uber-style wallet</span><h2>Payment methods</h2></div></div>{methods.map(item=><article key={item.id}><span className="payment-symbol">M</span><div><strong>{item.label}</strong><small>{item.is_default?"Default payment method":item.method_type}</small></div></article>)}<div className="payment-add"><input value={phone} onChange={event=>setPhone(event.target.value)} placeholder="M-Pesa phone number"/><button disabled={phone.replace(/\D/g,"").length<10} onClick={()=>void add()}>Add M-Pesa</button></div>{notice&&<small>{notice}</small>}<p>Card details are accepted only through a configured tokenization provider and are never stored by Mafundi.</p></div>
}
