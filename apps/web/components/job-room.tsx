...
export function JobRoom({ jobId }:{jobId:string}) {
  ...
  const pendingQuotes=room.quotes.filter(item=>item.status==="pending");
  const lowest=pendingQuotes.length?Math.min(...pendingQuotes.map(item=>item.amount)):0;
  const fastest=pendingQuotes.length?Math.min(...pendingQuotes.map(item=>item.eta_hours)):0;
  const highestRated=pendingQuotes.length?Math.max(...pendingQuotes.map(item=>item.artisan_rating)):0;
    ...
    <div className="room-grid">
      <section className="room-main">
        <article className="room-card"><div className="room-card-head"><div><span className="kicker">Compare offers</span><h2>Artisan quotes</h2></div><span>{room.quotes.length} received</span></div>
          {room.quotes.length ? <div className="quote-list">{room.quotes.map(item=><article key={item.id} className={item.status==="pending"&&item.artisan_rating===highestRated?"recommended-quote":""}><div><strong>{item.artisan_name}<BadgeCheck size={14}/></strong><span><Star size={12} fill="currentColor"/>{item.artisan_rating || "New"} · <Clock3 size={12}/>{item.eta_hours}h arrival</span><span className="quote-badges">{item.status==="pending"&&item.amount===lowest&&<b>Best price</b>}{item.status==="pending"&&item.eta_hours===fastest&&<b>Fastest</b>}{item.status==="pending"&&item.artisan_rating===highestRated&&<b>Top rated</b>}</span></div><strong>KSh {item.amount.toLocaleString()}<small>Estimated total KSh {(item.amount*1.1).toLocaleString()} incl. protection</small></strong><p>{item.message}</p>{client && item.status==="pending" && <button onClick={()=>void act(`quotes/${item.id}/accept`,"POST")} className="button button-dark">Choose this artisan</button>}<em className={item.status}>{item.status}</em></article>)}</div>:<Empty text="No artisan has quoted yet."/>}
...
function ReviewForm({act,jobId}:{act:(url:string,method:string,body?:object)=>Promise<boolean>;jobId:string}){const[rating,setRating]=useState(5);const[comment,setComment]=useState("");return <form className="room-card review-form" onSubmit={e=>{e.preventDefault();void act(`jobs/${jobId}/reviews`,"POST",{rating,comment})}}><span className="kicker">Verified review</span><h2>Rate completed work</h2><div className="star-picker" role="radiogroup" aria-label="Work rating">{[1,2,3,4,5].map(n=><button key={n} type="button" role="radio" aria-checked={rating===n} aria-label={`${n} star${n===1?"":"s"}`} onClick={()=>setRating(n)}><Star fill={n<=rating?"currentColor":"none"}/></button>)}</div><p className="star-picker-label">{rating===5?"Excellent":rating===4?"Very good":rating===3?"Good":rating===2?"Needs improvement":"Poor"}</p><textarea required value={comment} onChange={e=>setComment(e.target.value)} placeholder="How did the work go?"/><button className="button button-dark">Publish review</button></form>}
