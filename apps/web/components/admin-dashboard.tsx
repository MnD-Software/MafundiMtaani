...
export function AdminDashboard({initialSection="overview"}:{initialSection?:string}) {
  ...
  const nav = [
    ...
    ["people","Clients & teams",UsersRound],["coverage","Coverage map",MapPinned],["finance","Revenue & payouts",CircleDollarSign],["risk","Trust & fraud",ShieldCheck],
  ] as const;
...
function RiskCentre(){
  const[risks,setRisks]=useState<{id:string;signal_type:string;severity:string;score:number;status:string}[]>([]);const[documents,setDocuments]=useState<{id:string;document_type:string;status:string;provider:string;confidence:number}[]>([]);const[recon,setRecon]=useState<{invoice_count:number;un_invoiced:number;payments:{status:string;count:number;amount:number}[]}|null>(null);
  useEffect(()=>{void Promise.all([fetch("/api/marketplace/admin/risk-signals"),fetch("/api/marketplace/admin/document-verifications"),fetch("/api/marketplace/admin/reconciliation")]).then(async([a,b,c])=>{if(a.ok)setRisks(await a.json());if(b.ok)setDocuments(await b.json());if(c.ok)setRecon(await c.json())})},[]);
  return <section className="ops-data-page"><div className="ops-metrics"><Metric label="Open risk signals" value={risks.filter(item=>item.status==="open").length} note="Rule-based detection"/><Metric label="Pending documents" value={documents.filter(item=>item.status==="pending").length} note="Verification queue"/><Metric label="Invoices" value={recon?.invoice_count||0} note="Payment-backed records"/><Metric label="Un-invoiced" value={recon?.un_invoiced||0} note="Needs reconciliation"/></div><div className="ops-grid"><section className="analytics-card compliance-list"><span className="kicker">Fraud controls</span><h2>Risk signals</h2>{risks.length?risks.map(item=><article key={item.id}><strong>{item.signal_type.replaceAll("_"," ")}</strong><span>{item.severity} · score {item.score}</span></article>):<p>No risk signals.</p>}</section><section className="analytics-card compliance-list"><span className="kicker">Identity controls</span><h2>Document verification</h2>{documents.length?documents.map(item=><article key={item.id}><strong>{item.document_type.replaceAll("_"," ")}</strong><span>{item.status} · {item.provider}</span></article>):<p>No document records.</p>}</section></div></section>
}
