...
export function JobForm() {
  ...
  const [form, setForm] = useState({ trade: "Plumbing", title: "", description: "", area: "Kilimani", urgency: "today", scheduledFor:"", preferredArtisanId:"", budgetMin: "", budgetMax: "", name: "", phone: "" });
  const update = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value }));
  useEffect(()=>{try{const params=new URLSearchParams(window.location.search);const draft=localStorage.getItem("mafundi-job-draft");if(draft){const parsed=JSON.parse(draft);setForm(current=>({...current,...parsed.form,preferredArtisanId:params.get("artisan")||parsed.form?.preferredArtisanId||""}));setStep(Math.min(2,Math.max(0,parsed.step||0)))}else if(params.get("artisan"))setForm(current=>({...current,preferredArtisanId:params.get("artisan")||""}))}catch{/* Ignore damaged local drafts. */}},[]);
  ...
  const submitJob = async () => {
    ...
    try {
      const response = await fetch("/api/jobs", {
        ...
        body: JSON.stringify({ client_name: form.name, client_phone: form.phone, trade: form.trade, title: form.title, description: form.description, area: form.area, urgency: form.urgency === "week" ? "this_week" : form.urgency, scheduled_for:form.urgency==="scheduled"&&form.scheduledFor?new Date(form.scheduledFor).toISOString():null, preferred_artisan_id:form.preferredArtisanId||null, budget_min: Number(form.budgetMin || 0), budget_max: Number(form.budgetMax || 0) })
      });
  ...
  return <main className="job-shell">
    <div className="job-layout">
      ...
      <form className="job-form" onSubmit={(e) => { e.preventDefault(); if (step < 2) setStep(step + 1); else void submitJob(); }}>
        ...
        {step === 1 && <div className="form-panel"><h2>Where and when?</h2><button type="button" className="live-area-button" onClick={live.start}><Navigation size={16}/>{live.state==="live"?`Live area: ${live.area}`:live.state==="locating"?"Finding your area…":"Use my live location"}</button><label>Estate or neighbourhood<div className="input-icon"><MapPin size={18} /><select value={form.area} onChange={(e) => update("area", e.target.value)}>{!nairobiEstates.includes(form.area)&&<option>{form.area}</option>}{nairobiEstates.map((estate) => <option key={estate}>{estate}</option>)}</select></div></label><label>When do you need help?<div className="choice-grid">{[["today","Today"],["week","This week"],["scheduled","Choose a date"]].map(([value, label]) => <button type="button" className={form.urgency === value ? "selected" : ""} onClick={() => update("urgency", value)} key={value}>{label}</button>)}</div></label>{form.urgency==="scheduled"&&<label>Appointment date and time<input required type="datetime-local" min={new Date().toISOString().slice(0,16)} value={form.scheduledFor} onChange={event=>update("scheduledFor",event.target.value)}/></label>}</div>}
